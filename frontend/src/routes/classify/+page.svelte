...
<div class="mx-auto p-4 md:p-5 w-full md:w-3/5">
    <form method="post" enctype="multipart/form-data" use:enhance={({ formData }) => {
        ...
        <div class="w-full bg-slate-700 p-4 md:p-5 mb-4 md:mb-5">
            <h1 class="text-xl md:text-2xl">Classify files</h1>
            <h2 class="mt-2 text-sm md:text-base">Upload a file to classify it on the RCST framework</h2>
            {#if waitingfordata}
            <div class="bg-slate-500 p-3 mt-4 md:mt-5">
                Uploading... &nbsp;&nbsp;
                <svg class="inline-block" width="20px" height="20px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 150">
            ...
            <div class="bg-green-500 p-3 mt-4 md:mt-5">Upload succeeded!</div>
            {:else if form?.error}
            <div class="error bg-red-500 p-3 mt-4 md:mt-5">Upload failed with error: {form.error}</div>
            {/if}
            {#if form?.classification}
            <div class="mt-5 p-4 md:p-5 bg-slate-700">
                <h2 class="text-lg md:text-xl">Classification Result</h2>
                <p class="mt-2 text-sm md:text-base"><strong>Classification:</strong> {form.classification}</p>
            </div>
            {/if}
        </div>
        <div class="flex items-center justify-center w-full">
            <label for="file" class="flex flex-col items-center justify-center w-full h-48 md:h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                ...
                <input style="display:none" id="file" type="file" name="file" accept=".docx,.pdf,.pptx,.xlsx"/>
        ...
        <div class="flex items-center my-4 md:my-5">
            <button class="hover:bg-orange-500 rounded-sm bg-slate-700 p-3 text-sm md:text-base" href='/'>Submit</button>
            <div class="grow"></div>
        </div>
    </form>
</div>