...
<div class="w-full sticky top-0 z-50 bg-slate-900 shadow-lg">
    <div class="w-90 px-10 py-2 flex items-center justify-between bg-slate-900">
        <div class="flex items-center justify-between px-4 py-2">
            ...
            {#if loggedIn}
                <div class="hidden sm:block text-white rounded-sm ml-5 p-3 text-sm sm:text-base">
        ...
        <div class="hidden lg:flex items-center space-x-4">
            ...
            {#if !loggedIn}
            <a class="hover:bg-orange-500 text-white bg-slate-700 rounded-lg p-3 duration-300m" href="/login">Login</a>
            {/if}
        </div>
