...
<div>
    <div class="w-90 px-10 h-10border-b-2 flex items-center bg-slate-900">
        ...
        {#if loggedIn}
        <div class="hidden md:inline hover:bg-orange-500 rounded-sm bg-slate-800 p-3 m-2">welcome, {username}!</div>
        {/if}
        <div class=grow></div>
        ...
        <div class={`lg:hidden ${mobile ? 'block' : 'hidden'} absolute top-0 left-0 w-full bg-slate-900 pb-5 px-10 z-10`}>
            <div class="flex items-center">
                <div class="rounded-sm bg-slate-700 p-3 m-2">NYP CNC AI</div>
                <button aria-label=items on:click={() => mobile = !mobile} class="ml-auto text-white">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-6 w-6">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
                    </svg>
                </button>
